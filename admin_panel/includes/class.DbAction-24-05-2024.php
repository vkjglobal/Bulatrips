...
class DbAction {
    ...
    public function selectByLIMITAgents($tableName,$id,$offset='') {
        $role   =   2;// agents only        
        $query = "SELECT  `id`,`first_name`,`last_name`,`email`,`mobile`,`agent_status`,`credit_balance`,`image`  FROM $tableName WHERE role = ? ORDER BY $id DESC $offset";  
         $params = [$role];
        return $this->executeQuery($query,$params);
    }
...
public function updateAgentBalance($agentId,$amountnew){
        // echo $agentId;exit;
         $query = "UPDATE users SET credit_balance = ? WHERE id = ?";
    ...
    // nimmi update the status value 0 to $status - 17-05-2024
