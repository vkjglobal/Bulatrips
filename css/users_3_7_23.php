...
foreach ($datas as $row) {
   $i++;
    $filteredData[] = [$i,$row['first_name']." ".$row['last_name'],$row['email'],$row['mobile'],$row['username'],$row['status'] ];
...
<script>
  ...
  var currentPage = 1;
  var itemsPerPage = 5; // Adjust this value based on the number of rows to display per page
  var sortColumn = -1;
  var sortDirection = 1;
