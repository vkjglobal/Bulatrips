...
<?php
  ...
  //  print_r($bookCanusers);
    ...
    $return_dep_date      =   $objCancel->ReturnDepDate($bookingId,$userId,$arrival_location);
     ...
     $cabin_preference_return    =   $return_dep_date[0]['cabin_preference'];
     $airline_code_return    =   $return_dep_date[0]['airline_code'];
     $flight_no_return    =   $return_dep_date[0]['flight_no'];
...
?>
    <section>
        <div class="container">
            ...
            <input type="hidden" name="selected-flights" id="flight-num" value="<?php echo $resultBooking[0]['flight_no']; ?>">
                                ...
                                <div class="col-12 mb-3">
                                    <div class="d-flex justify-content-start">
                                        ...
                                        <div class="d-flex align-items-center chkbx ml-3">
                                            <input type="checkbox" name="selected-flights_show" checked value="<?php echo $resultBooking[0]['flight_no']?>" id="flight-number" disabled="disabled">
            ...
            <div class="row fs-13 mb-3 px-3" id="flght_options">
             <!-- ======================================= -->
...
<?php
//###########################################reissue result after choosing flight option with  reissue button###########
 if(isset($_POST['PreferenceOption'])){
     //echo "JJ";exit;
    ...
    "Message": "Your request for reissue has been submitted successfully. Our team will process it within 60 minutes."
        ...
        if(!empty($responseData['Message'])){                           
            $message    =   $responseData['Message'];
             $ticktinprocess_msg =  $message;
                                         echo '<script>';
                            echo 'document.addEventListener("DOMContentLoaded", function() {';
                            echo '    $("#TicketinMessage").text("' . $ticktinprocess_msg . '");';
                            echo '    $("#TicketinprocessErr").modal("show");';
                            echo '});';
                            echo '</script>';
        ...
        //may alreay cancelled 
                         $response_New = array( 'status' => 'error', // You can set this to 'error' in case of an error
                        'message' => $message );
                $objCancel->_writeLog('step data empty of accept reissue'.$message,'searchreissue.txt');
     }
...
//===============================
    ...
    $(document).ready(function() {
       ...
       $("#flight-search_reissue").on("submit", function(event) {
        ...
        return flase; // Prevent further execution
    }
         $('#psngr-error').remove(); 
        ...
        if (responseData.status === 'success') {
           ...
           //  return false;
            ...
            const successMessage = responseData.message;
                if (responseData.response_preference && Object.keys(responseData.response_preference).length > 0) {
                    //################## need loader 
                    //########################
                    const flghtOptionsDiv = $("#flght_options");
                    const htmlContentNew    = responseData.html_div;
                    // Clear the existing content
                    flghtOptionsDiv.empty();
                     $("#flght_options").html(htmlContentNew);
                     return false;
                    // Create HTML content for the entire flight options list
   
                    }
                /*    else {
                            // Display a message when no response_preference data is present
                            $("#flght_options").html("No flight options available.");
                        } */
                   else if (responseData.ptr_status === 'InProcess'){
                         ...
                         $("#close_ticketinprocessErr").click(function() {
                          ...
                          alert ("heelo");return false;
                            //ptr status completed
                            //***********************call GetEXChangeQuote APi**************************** */
                            }
          }
          else {
              //alert("YYY");return false;
           // console.log("error from reissue quote api response");
