...
// print_r($passengers);
 $ticket_num    =        $passengers[0]['eticket'];
...
//print_r($response);  
    ...
    //****************************************************************
        ...
        $logReQ =   print_r($requestData, true);
            $objCancel->_writeLog('-------------'.date('l jS \of F Y h:i:s A').'-------------','refund.txt');
                    $objCancel->_writeLog('REsponse Received for MF:\n'.$mfreNum,'refund.txt');
                      $objCancel->_writeLog('userId is '.$userId,'refund.txt');
                      $objCancel->_writeLog('Booking ID is '.$bookingId,'refund.txt');
...
if(isset($responseData['Success']) && $responseData['Success']) {
            ...
            $objCancel->_writeLog('Step 1Success '.$PTRStatus,'refund.txt');
...
//$PTRStatus	= "completed";	
             if($PTRStatus == "InProcess"){
			             $message1   =   "Your Canellation is :".$PTRStatus." This will update within ". $hours." Hours";
                $bookCanIns      =   $objCancel->insCncelSts($bookingId,$userId,$precancelsts,$errorCode ='', $mfreNum,$traceId='',$httpCode,$PTRId,$PTRType,$SLAInMinutes,$PTRStatus,$VoidingWindow='', $ticket_num,$AdminCharges='' ,$GSTCharge='',$TotalVoidingFee='',$per_psnger_refund_amnt=$TotalRefundAmount,$Currency='',$cancel_status,$message='');                                                   

             }
           else if($PTRStatus == "completed" )
           {
	//$cancel_status	=1;
          // $message1   =   "Your Canellation is :".$PTRStatus." Total Refundable Amount is : ". $Currency." ".$TotalRefundAmount;
              $message1   =   "Your Canellation is :".$PTRStatus." Total Refundable Amount is : $".$TotalRefundAmount;
                $bookCanIns      =   $objCancel->insCncelSts($bookingId,$userId,$precancelsts,$errorCode ='', $mfreNum,$traceId='',$httpCode,$PTRId,$PTRType,$SLAInMinutes,$PTRStatus,$VoidingWindow='', $ticket_num ,$AdminCharges='' ,$GSTCharge='',$TotalVoidingFee='',$per_psnger_refund_amnt=$TotalRefundAmount,$Currency='',$cancel_status,$message='');                                                   

         
           }
        //  print_r($bookCanIns);exit;
                              $response_New = array(
    ...
    foreach ($responseData['Data']['Errors'] as $error) {
       ...
       $message_new = $errorMessage;
        ...
        $message    = "Problem in Cancellation";
          $cancel_status = 0;
            ...
            'message' => $message
                      ...
                      $bookCanIns      =   $objCancel->insCncelSts($bookingId,$userId,$precancelsts,$errorCode ='', $mfreNum,$traceId='',$httpCode,$PTRId='',$PTRType='Refund',$SLAInMinutes='',$PTRStatus='',$VoidingWindow='', $ticket_num=''  ,$AdminCharges='' ,$GSTCharge='',$TotalVoidingFee='',$TotalRefundAmount='',$Currency='',$cancel_status,$message_new);                                                   

         $objCancel->_writeLog('Step data of errors  '.$errorMessage,'refund.txt');
        ...
        else if($httpCode !=200)
        {
         ...
         $objCancel->_writeLog('step httpcode not 200 '.$message,'refund.txt');
        }
                        ...
                        //Booking is not eligible for voiding. -may be not under void window param
                         $response_New = array(
                            ...
                            'message' => $message
                                      ...
                                      $objCancel->_writeLog('step data empty '.$message,'refund.txt');
         ...
         $objCancel->_writeLog('step end of Refund===== '.$message,'refund.txt');
